...
def copifier(stripped_user_dir):
    ...
    # old ftp commented out
    # shutil.copy(os.path.join('V:\\ZIP HOMEWORKER', stripped_user_dir, 'pickup.zip'),
    #            os.path.join('X:\\', stripped_user_dir))
    # print("Copying to FTP...")
    if stripped_user_dir == 'shenricks':
        shutil.copy(os.path.join('V:\\ZIP HOMEWORKER', stripped_user_dir, 'pickup.zip'),
                    os.path.join('\\\\SDS-SRV300', 'shendricks', 'pickup.zip'))
    else:
        shutil.copy(os.path.join('V:\\ZIP HOMEWORKER', stripped_user_dir, 'pickup.zip'),
                    os.path.join('\\\\SDS-SRV300', stripped_user_dir, 'pickup.zip'))
    print("Copying to SRV300 FTP...")
...
def idc_image_firstlast(idc_number):
    ...
    return (image_type, first_image_number, last_image_number, employer_count)
...
def user_experience():
    ...
    while True:
        ...
        print('Batch Type:', idc_image_firstlast(line_2_clean)[0],
              ...
              'Employer Count:', idc_image_firstlast(line_2_clean)[3], )
